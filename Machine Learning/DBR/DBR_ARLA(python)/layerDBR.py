...
def calFWHM(R,wavelength,tarwave):
    ...
    tarhi = list(i for i in range(taridx,wavelength.shape[1],1) if R[i] < 0.5*tarint)[0]
    tarlo = list(i for i in range(taridx,0,-1) if R[i] < 0.5*tarint)[0]
...
def reward(R, tarwave, half_bandwidth, wavelength):
    ...
    lb_idx = np.where(wavelength == lbound)[1][0]
    ub_idx = np.where(wavelength == ubound)[1][0]

    # R_in = np.sum(R[lb_idx:ub_idx+1])
    # R_out = np.sum(np.hstack((R[0:lb_idx+1], R[ub_idx:])))
...
def step(s, action, ubound, lbound, INPUT_SIZE):

...
class DQN:
    ...
    def _build_network(self, l_rate=1E-6) -> None:
        # hiddenlayer's number and length
        Hidden_Layer = np.array([250, 250, 250, 250])
        ...
        with tf.variable_scope(self.net_name):
            ...
            for i in range(num_layer):
                ...
                net = tf.layers.dense(
                    net, Hidden_Layer[i], activation=tf.nn.elu,
                    kernel_initializer=tf.contrib.layers.xavier_initializer(),
                    bias_initializer=tf.contrib.layers.xavier_initializer())

            net = tf.layers.dense(
                net, self.output_size,
                kernel_initializer=tf.contrib.layers.xavier_initializer(),
                bias_initializer=tf.contrib.layers.xavier_initializer())
            self._Qpred = net

            self._Y = tf.placeholder(
                tf.float32, shape=[None, self.output_size], name="output_y")
    ...
    def updatewTboard(self, x_stack: np.ndarray, y_stack: np.ndarray) -> list:
        ...
        return self.session.run([self.merged_summary, self._train], feed)
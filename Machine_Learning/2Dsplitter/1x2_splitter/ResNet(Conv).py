...
def main(n_batch, lr_rate, beta1, beta2):
    ...
    # input: ? * 20 * 10 * 1
    # output: ? * 20 * 10 * 16
    res1 = residual_block(relu, 16, False, is_training)
    res2 = residual_block(res1, 16, False, is_training)
    res3 = residual_block(res2, 16, False, is_training)
    res4 = residual_block(res3, 16, False, is_training)
    res5 = residual_block(res4, 16, False, is_training)

    # input: ? * 20 * 10 * 16
    # output: ? * 10 * 5 * 32
    res6 = residual_block(res5, 32, True, is_training)
    res7 = residual_block(res6, 32, False, is_training)
    res8 = residual_block(res7, 32, False, is_training)
    res9 = residual_block(res8, 32, False, is_training)
    res10 = residual_block(res9, 32, False, is_training)

    # input: ? * 10 * 5 * 32
    # output: ? * 5 * 3 * 64
    res11 = residual_block(res10, 64, True, is_training)
    res12 = residual_block(res11, 64, False, is_training)
    res13 = residual_block(res12, 64, False, is_training)
    res14 = residual_block(res13, 64, False, is_training)
    res15 = residual_block(res14, 64, False, is_training)
    ...
    with tf.Session() as sess:
        ...
        for n in range(int(Nlearning*n_copy/n_batch)):
            ...
            feed_train = {X: feed_trainX, Y: feed_trainY, is_training: True}
        ...
        for n in range(int(Ntest/test_batch)):
            ...
            feed_test = {X: feed_testX, Y: feed_testY, is_training: False}
...
def residual_block(X, num_filter, chg_dim, is_training):
    stride = 1
    ...
    bm1 = tf.layers.batch_normalization(inputs=conv1, trainable=False)
    ...
    bm2 = tf.layers.batch_normalization(inputs=conv2, trainable=False)

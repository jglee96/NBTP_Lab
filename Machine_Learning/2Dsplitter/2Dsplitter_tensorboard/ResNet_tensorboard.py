...
def main(n_batch, lr_rate, beta1, beta2, n_hidden):
    ...
    for i, n in enumerate(n_hidden):
        ...
        with tf.name_scope('dense'+str(i)+'_1'):
            ...
            net_hist.append(tf.summary.histogram("activations"+str(i)+'_1', net))
            dense_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "dense"+str(i)+'_1')
            net_hist.append(tf.summary.histogram("weights"+str(i)+'_1', dense_vars[0]))
            net_hist.append(tf.summary.histogram("biases"+str(i)+'_1', dense_vars[1]))
        with tf.name_scope('dense'+str(i)+'_2'):
            ...
            net_hist.append(tf.summary.histogram("activations"+str(i)+'_2', net))
            dense_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "dense"+str(i)+'_2')
            net_hist.append(tf.summary.histogram("weights"+str(i)+'_2', dense_vars[0]))
            net_hist.append(tf.summary.histogram("biases"+str(i)+'_2', dense_vars[1]))
    ...
    learning_rate = tf.train.exponential_decay(
        lr_rate, global_step, 500, 0.95, staircase=True)

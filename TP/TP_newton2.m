...
R23 = zeros(length(f),1);
T1_th = zeros(length(f),1);
T2_th = zeros(length(f),1);
T3_th = zeros(length(f),1);
...
for w=point:length(f)
    ...
    while error>epsilon
        ...
        T1_th(w) = T12(w)*T23(w)*exp(-1i*k0(w)*n(w)*d);
        T2_th(w) = T12(w)*R23(w)^2*T23(w)*exp(-1i*k0(w)*n(w)*d*3);
        T3_th(w) = T12(w)*R23(w)^4*T23(w)*exp(-1i*k0(w)*n(w)*d*5);
        T_th(w) = T1_th(w)+T2_th(w)+T3_th(w);
        
        df = T_th(w) - T_ex(w);
        
        df_diff = (4*exp(-d*k0(w)*n(w)*1i))/(n(w) + 1)^2 + (4*exp(-d*k0(w)*n(w)*3i)*(n(w) - 1)^2)/(n(w) + 1)^4 + (4*exp(-d*k0(w)*n(w)*5i)*(n(w) - 1)^4)/(n(w) + 1)^6 - (8*n(w)*exp(-d*k0(w)*n(w)*1i))/(n(w) + 1)^3 + (4*n(w)*exp(-d*k0(w)*n(w)*3i)*(2*n(w) - 2))/(n(w) + 1)^4 - (16*n(w)*exp(-d*k0(w)*n(w)*3i)*(n(w) - 1)^2)/(n(w) + 1)^5 + (16*n(w)*exp(-d*k0(w)*n(w)*5i)*(n(w) - 1)^3)/(n(w) + 1)^6 - (24*n(w)*exp(-d*k0(w)*n(w)*5i)*(n(w) - 1)^4)/(n(w) + 1)^7 - (d*k0(w)*n(w)*exp(-d*k0(w)*n(w)*1i)*4i)/(n(w) + 1)^2 - (d*k0(w)*n(w)*exp(-d*k0(w)*n(w)*3i)*(n(w) - 1)^2*12i)/(n(w) + 1)^4 - (d*k0(w)*n(w)*exp(-d*k0(w)*n(w)*5i)*(n(w) - 1)^4*20i)/(n(w) + 1)^6;
